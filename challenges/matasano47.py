...
def get_small_rsa_key(bitlength, e):
    ...
    '''
    print 'using bitlength: ' + str(bitlength)
...
def pkcs1v15_oracle(encrypted, n, d, bl=bitlength):
    ...
    '''
    decrypted = bi2ba(decrypt(n, d, encrypted, bl), fixed=bl/8)
    return True if decrypted[:2]=='\x00\x02' else False


def pkcs1v15_pad_for_encryption(m, bl=bitlength):
...
if __name__ == '__main__':
    ...
    while True:
        ...
        c1 = c * encs1 % n
        oracle_check = pkcs1v15_oracle(c1, n, d)
        if oracle_check:
            break
        s1 += 1
    ...
    while len(M)>1 or M[-1][1] != M[-1][0]:
        ...
        while not found:
            ...
            for s in range(int((2*B + r*n)/b), int((3*B + r*n)/a)+1):
                ...
                if pkcs1v15_oracle(c * encs % n, n, d):
                    found = True
                    break
